...
export default function BlogCard1({ blog }) {
  ...
  return (
    ...
    {similarTransition((style, item) =>
        <animated.div style={style} className='w-[570px] h-[130px] hover:text-white hover:bg-black rounded-lg bg-white pb-10 transtion-[0.2s] aniate-bounce blogcaranim' onMouseEnter={() => setHovering(true)} onMouseLeave={() => setHovering(false)}>
          <div className='grid grid-flow-col grid-cols-6'>
            <div className='w-[280x] h-[130px] col-span-2'>
          <img src={imgFile} alt="" className='w-[280x] h-[130px]' />

            </div>
          <div className='ml-2 col-span-4'>
              <span className='font-bold text-[18px] transition-[0.2s]'>{blog?.title}</span> <br />
